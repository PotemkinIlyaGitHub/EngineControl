...
private:
  const uint8_t _pin;
  uint8_t _counter = 0; // Счетчик для определения нажатия
  const uint8_t _countShortPressed = 5; // Количество тиков таймера для определения короткого нажатия
  const uint8_t _countLongPressed = 150;// Количество тиков таймера для определения длинного нажатия
